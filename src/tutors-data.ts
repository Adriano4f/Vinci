...
interface NxTutor {
  slug: string;      // URL key — stable id for links and lookups
  name: string;
  spec: string;      // short specialty label shown on cards
  years: number;
  level: string;     // academic levels the tutor covers
  rating: number;
  students: number;  // students attended so far
  img: string;
  bio: string;       // short bio for the directory card
  bioLong: string;   // extended description for the profile page
  subjects: string[]; // must match SUBJECTS names in tutorias.ts
  hours: string;
}

const NEXUM_TUTORS: NxTutor[] = [
  {
    slug: "james-carter",
