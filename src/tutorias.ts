...
(() => {
  ...
  document
    .querySelectorAll<HTMLAnchorElement>('a[href^="tutorias.html?plan="]')
    .forEach((a) => {
      a.addEventListener("click", (e) => {
        e.preventDefault();
        const slug = new URL(a.href).searchParams.get("plan") ?? "";
        if (planSelect && planBySlug(slug)) planSelect.value = slug;
        syncSummary();
